...
class ComplexLiteral_gate_text(torch.nn.Module):
    ...
    def __init__(self, num_entities, num_relations, numerical_literals, text_literals):
        ...
        self.emb_num_lit_real = GateMulti(self.emb_dim, self.n_num_lit, self.n_txt_lit)
        self.emb_num_lit_img = GateMulti(self.emb_dim, self.n_num_lit, self.n_txt_lit)
        # Dropout + loss
        self.inp_drop = torch.nn.Dropout(Config.input_dropout)
        self.loss = torch.nn.BCELoss()
    ...
    def forward(self, e1, rel):
        ...
        e1_num_lit = self.numerical_literals[e1.view(-1)]
        e1_txt_lit = self.text_literals[e1.view(-1)]
        e1_emb_real = self.emb_num_lit_real(e1_emb_real, e1_num_lit, e1_txt_lit)
        e1_emb_img = self.emb_num_lit_img(e1_emb_img, e1_num_lit, e1_txt_lit)

        e2_multi_emb_real = self.emb_num_lit_real(self.emb_e_real.weight, self.numerical_literals, self.text_literals)
        e2_multi_emb_img = self.emb_num_lit_img(self.emb_e_img.weight, self.numerical_literals, self.text_literals)
...
class DistMultLiteral_text(torch.nn.Module):

    def __init__(self, num_entities, num_relations, numerical_literals, text_literals):
        super(DistMultLiteral_text, self).__init__()
        ...
        # LiteralE's g
        self.emb_lit = torch.nn.Linear(self.emb_dim+self.n_num_lit+self.n_txt_lit, self.emb_dim)
    ...
    def forward(self, e1, rel):
        ...
        e1_num_lit = self.numerical_literals[e1.view(-1)]
        e1_txt_lit = self.text_literals[e1.view(-1)]
        e2_multi_emb = self.emb_lit(torch.cat([self.emb_e.weight, self.numerical_literals, self.text_literals], 1))
        # --------------
        # End literals
