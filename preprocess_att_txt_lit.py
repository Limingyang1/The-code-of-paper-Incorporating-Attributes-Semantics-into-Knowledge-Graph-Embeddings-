...
# Load raw literals
df = pd.read_csv(f'data/{args.dataset}/literals/att_txt.txt', header=None, sep='\t')
...
for ent, txt in tqdm(zip(df[0].values, df[1].values)):
...
np.save(f'data/{args.dataset}/literals/att_txt_literals.npy', txt_lit)
